...
export { config, validatePurchase, getPurchaseData };
...
function getPurchaseData(purchase, options) {
    ...
    if (options && options.ignoreExpired) {
        var now = Date.now();
        var expiry = purchase.expiryTimeMillis || purchase.expirationTime;
        if (purchase.lineItems && purchase.lineItems[0]) {
            expiry = purchase.lineItems[0].expiryTime || expiry;
        }
        if (expiry && now - new Date(expiry).getTime() >= 0) {
            return [];
        }
    }
    var data = [];
    ...
    purchaseInfo.purchaseDate = purchase.startTime || purchase.purchaseTime;
    ...
    if (purchase.lineItems && purchase.lineItems[0]) {
        purchaseInfo.expirationDate = purchase.lineItems[0].expiryTime || purchaseInfo.expirationDate;
    }
